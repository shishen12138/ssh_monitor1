...
echo "=== 下载模块文件（从 main/ 目录重命名到子目录） ==="

# host_manager
mkdir -p host_manager
wget -O host_manager/manager.py "$REPO_URL/host_manager.py"

# ssh_client
mkdir -p ssh_client
wget -O ssh_client/ssh.py "$REPO_URL/ssh_client.py"

# monitoring
mkdir -p monitoring
wget -O monitoring/monitor.py "$REPO_URL/monitoring.py"

# aws_importer
mkdir -p aws_importer
wget -O aws_importer/importer.py "$REPO_URL/aws_importer.py"

# logger
mkdir -p logger
wget -O logger/logger.py "$REPO_URL/logger.py"

# web_panel
mkdir -p web_panel
wget -O web_panel/routes.py "$REPO_URL/web_routes.py"
wget -O web_panel/ws_monitor.py "$REPO_URL/ws_monitor.py"
wget -O web_panel/ws_logs.py "$REPO_URL/ws_logs.py"

# components (前端 Vue)
mkdir -p components
wget -O components/HostMonitorFull.vue "$REPO_URL/HostMonitorFull.vue"
